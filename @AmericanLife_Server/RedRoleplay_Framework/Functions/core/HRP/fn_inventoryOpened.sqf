...
HRP_fnc_inventoryOpened = {
	...
	if(_crate isKindOf "Man" && !alive _crate) exitWith {
		["Error", "You are not allowed to loot dead bodies", false] spawn HRP_fnc_Notifications;
		handle = true;
		handle;
	};
	...
	if(_crate isKindOf "LandVehicle" || _crate isKindOf "Ship" || _crate isKindOf "Air") exitWith {
		if(!(_crate in HRP_Cars) && {(locked _crate) == 2}) exitWith {
			["Error","You're not allowed to open the vehicles inventory while it's locked", false] spawn HRP_fnc_Notifications;
			handle = true;
			handle;
		};
	};
