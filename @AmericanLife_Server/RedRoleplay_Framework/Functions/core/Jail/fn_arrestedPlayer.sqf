...
HRP_fnc_arrestedPlayer = {
	...
	player setVariable ["arrestTime", (_timeLeft * 60), true];
	...
	while {true} do
	{
		...
		if (player distance (getMarkerPos "HRP_Prison") > 50) exitWith {
			...
			["Escaped", "You have Escaped", true] spawn HRP_fnc_Notifications;
			123 cutText	["", "PLAIN"];
		};

		if (_timeLeft <= 0) exitWith {
			["Released", "You have been Released", true] spawn HRP_fnc_Notifications;
			player setVariable ["Arrested", false, true];
			player setVariable ["arrestTime", 0, true];
			player setVariable ["arrestNotes", nil, true];
