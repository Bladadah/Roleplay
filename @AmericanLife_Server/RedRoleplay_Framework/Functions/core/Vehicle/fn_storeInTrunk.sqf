...
HRP_fnc_storeInTrunk = {
	...
	if (count _trunkItems > _target getVariable ["maxCapacity", 8]) exitWith { ["Error", "Your trunk is already full!", true] spawn HRP_fnc_Notifications;};
	...
	["Success", "Item Stored in Trunk", true] spawn HRP_fnc_Notifications;
};
