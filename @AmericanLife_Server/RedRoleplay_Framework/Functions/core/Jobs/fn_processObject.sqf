...
HRP_fnc_processObject = {
	...
	if ((count _ammount) < _count) exitWith { ["Error", "You do not have that many of that item", true] spawn HRP_fnc_notifications; };

	{
		if (_object isEqualTo (_x select 0)) then {
			_toCount = _count;
			_message = format ["Processing %1 %2", _count, _x select 2];
			["Process Started", _message, true] spawn HRP_fnc_notifications;
			while {_toCount > 0} do
			{
			...
			if (Processed) exitWith { 
				_msg = format ["Processed %1 %2", _count, _x select 2];
				["Process Completed", _msg, true] spawn HRP_fnc_notifications;
			};
		};
