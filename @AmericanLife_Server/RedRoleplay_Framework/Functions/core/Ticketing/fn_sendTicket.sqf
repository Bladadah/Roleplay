...
HRP_fnc_sendTicket = {
	...
	_fields = [1004, 1402];
	{
		if ((ctrlText _x) isEqualTo "") exitWith { ["Error", "You must enter all fields", true] spawn HRP_fnc_Notifications; };
	} forEach _fields;

	if ((parseNumber (ctrlText 1401)) <= 0) exitWith {
		["Error", "You must enter a valid number", true] spawn HRP_fnc_Notifications
	};
