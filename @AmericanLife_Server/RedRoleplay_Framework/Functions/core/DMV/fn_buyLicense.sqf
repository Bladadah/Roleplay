...
HRP_fnc_buyLicense = {
	_text = ctrlText 1400;
	if ((count _text) < 5) exitWith { ["Error", "You must sign off this purchase!", true] spawn HRP_fnc_Notifications; };
	...
	if (_check) then {
		...
		["Success", "You have purchased this license!", true] spawn HRP_fnc_Notifications;
	} else {
		["Error", "Insufficient Funds", true] spawn HRP_fnc_Notifications;
	};
};