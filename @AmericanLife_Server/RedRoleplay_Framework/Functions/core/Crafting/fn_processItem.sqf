...
HRP_fnc_processItem = {
	...
	if !(isNil {HRP_processingUnit getVariable "unitInProcessing"}) exitWith { ["Error", "You are already processing materials in this machine", true] spawn HRP_fnc_Notifications; };
...

	{
		...
		if (_count < (_x select 1)) exitWith { ["Error","You do not have the required items", true] spawn HRP_fnc_Notifications; HRP_cancelledd = true;};
	...
	if !(HRP_cancelledd) then {
		...
		if (_count2 < count (_itemToCraft select 3)) exitWith { ["Error", "You do not have the required Machines nearby", true] spawn HRP_fnc_Notifications; };
		...
		["Success", "Pick your item up from the last used machine", true] spawn HRP_fnc_Notifications;
	};
};