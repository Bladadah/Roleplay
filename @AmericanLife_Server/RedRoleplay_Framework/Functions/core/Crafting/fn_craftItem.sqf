...
HRP_fnc_craftItem = {
	if (HRP_inProcessing) exitWith { ["Error", "You cannot smelt and craft at the same time!", true] spawn HRP_fnc_Notifications; };
	...
	_items = items player;
	{
		...
		if (_count < (_x select 1)) exitWith { ["Error","You do not have the required items", true] spawn HRP_fnc_Notifications; _bool = true;};
