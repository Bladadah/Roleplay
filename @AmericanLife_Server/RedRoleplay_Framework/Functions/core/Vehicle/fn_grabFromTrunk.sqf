...
HRP_fnc_grabFromTrunk = {
	...
	if (!isNil {player getVariable "carryingObject"}) exitWith { ["Error", "You cannot carry more than one item!", true] spawn HRP_fnc_Notifications; };

	_item = lbData [1500, ( lbCurSel 1500 )];
	if (_item isEqualTo "") exitWith { ["Error", "You must select an object", true] spawn HRP_fnc_Notifications; };
