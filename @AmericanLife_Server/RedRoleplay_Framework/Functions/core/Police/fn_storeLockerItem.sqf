...
HRP_fnc_storeLockerItem = {
	...
	if (isNil '_data') exitWith { ["Error", "You must select an item to store", true] spawn HRP_fnc_Notifications; };
	...
	if (count lockerGear > 50) exitWith {  ["Error", "Your Locker is already Full!", true] spawn HRP_fnc_Notifications; };
