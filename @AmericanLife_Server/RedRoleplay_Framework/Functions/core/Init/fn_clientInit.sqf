HRP_fnc_clientInit = {
	...
	player setVariable ["firstTime", false, true];
	[] call HRP_fnc_Configuration;

	HRP_startCar = selectRandom HRP_startingCars;
	[player, HRP_startCar] remoteExecCall ["Server_fnc_initStats", 2];
	sleep 1;
	[player] remoteExecCall ["Server_fnc_fetchGarageImmigration", 2];

	sleep 1;
	HRP_firstTime = player getVariable "firstTime";

	sleep 1;
	
	if (HRP_firstTime) then {
		player setVariable ["Identities", []];
