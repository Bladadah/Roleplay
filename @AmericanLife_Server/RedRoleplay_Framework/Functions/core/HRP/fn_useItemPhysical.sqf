...
HRP_fnc_useItemPhysical = {
		...
		switch (_value) do
		{
			case "Wheat Seeds":
			{
				if ((surfaceType position player) isEqualTo "#CRGrit1" || !(player inArea "Wheat_1" || player inArea "Wheat_3")) exitWith { ["Error", "You cannot grow wheat here", true] spawn HRP_fnc_Notifications; };
				[] spawn HRP_fnc_startWheatFarm;
			};
			case "Bandage":
			{
				[20, "RR_Bandage"] spawn HRP_fnc_healPlayer;
			};
			case "Plant Pot":
			{
