...
HRP_fnc_plantWeed = {
	...
	if !(((player distance (nearestBuilding player)) > 15) || (count (nearestObjects [player, ["Land_Portable_generator_F"], 20]) > 0)) exitWith { ["Error", "You must be near a building or have a generator nearby", true] spawn HRP_fnc_Notificiations; };
	player removeItem "RR_weedSeeds";
