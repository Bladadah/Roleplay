...
HRP_fnc_openLocker = {
	...
	if (_lockerType == "Cop" && (player getVariable "cop" <= 0)) exitWith { ["Error", "You must be on Duty to use this Locker", true] spawn HRP_fnc_Notifications; };
	...
	test = _lockerType;
	if !(found) exitWith { ["Error", "This locker is invalid", true] spawn HRP_fnc_Notifications; };
